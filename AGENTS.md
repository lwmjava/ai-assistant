...
`tasks.yaml` 中 `GOV-001`～`RAG-010` 已完成。下次拆任务时，先完成下面三项，一次只拆一个阶段：

```text
评审修复阶段 2：控制面权限、审计与单一当前版
→ 阶段 3：软删除与保留期
→ 阶段 4：版本状态机、跨租户审批、发布历史版
```

阶段 4 之后按交付排期，不再把切分、Embedding、独立 Reranker、Query Rewrite 排成连续数月的 RAG 深耕。总排期见 `docs/plans/plan_delivery_2027-03-25.md`：

```text
阶段 A 知识库治理（上列阶段 2、3、4，最高优先级）
→ 阶段 B 最小安装点
