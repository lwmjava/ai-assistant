...
> 说明：本文记录当前代码事实、差距、Evaluation 数据构建方法和可直接使用的提示词。日历推进顺序以 `docs/plans/plan_delivery_2027-03-25.md` 为准。代码持续变化时，应重新核对本文与真实实现。`GOV-001`～`RAG-010` 已在各自验收范围内完成；下次拆任务从阶段 A1（评审修复阶段 2）开始。
...
文中阶段 0～4 是历史方法与实验约束，不得再当当前日历待办。日历推进顺序见 `docs/plans/plan_delivery_2027-03-25.md`：阶段 A（知识库治理）→ B（最小安装点）→ C（MVP，2026-12-25 达到 80%）→ D（完整交付，2027-03-25 达到 100%）。下次拆任务从阶段 A1 开始。
...
`GOV-001`～`RAG-010` 已完成。日历顺序见 `docs/plans/plan_delivery_2027-03-25.md`。下次拆任务从阶段 A1 开始，一次只拆一个阶段：

```text
阶段 A 知识库治理（评审修复阶段 2 → 3 → 4，最高优先级）
→ 阶段 B 最小安装点（Compose / 健康检查含 DB 与向量库、租户与用户、对话检索与来源）
→ 阶段 C MVP（2026-12-25 达到约定范围的 80%）
→ 阶段 D 完整交付（2027-03-25 达到约定范围的 100%）
```

正式向量库目标为 Milvus（ADR-0002）；默认配置在第 5 节门槛通过前仍是 Local。闭环放在 B1 与 C6，不另开跨月向量库阶段。切分 / Embedding / 独立 Reranker / Query Rewrite 不排成连续数月深耕；交付排期至多在 D4 保留一轮。
