...
> **关联文档**：
> - `docs/product/as-is-capability-matrix.md`（当前能力矩阵与文档漂移）
> - `docs/architecture/target-agent-harness-architecture.html`（最终目标架构图与时序图可视化）
> - `docs/product/项目设计方案.md`（2026-08-07 历史设计稿，路径与默认值以 As-Is 矩阵为准）
> - `docs/governance/agent-harness-engineering.md`（Harness 治理）
> - `AGENTS.md`（当前代码事实入口）
>
> **阅读规则**：§3.1 是 TARGET。当前实现以 `AGENTS.md` 和 As-Is 矩阵为准。未标 `Implemented` 的能力不得当已交付宣传。
...
| **Agent 推理** | 可视化编排 | 工作流节点 | ReAct 循环 | 图状态机 | **当前：自研 AgentPipeline 五阶段（Implemented）；LangGraph Supervisor 可选（Partial）；TARGET 可保留图编排** |
| **RAG 检索** | 内置基础 | 深度优化 | 对话策展 | 需自建 | **当前：Dense + BM25 + RRF（Implemented）；独立 4 重排 Planned，不得写成已实现** |
...
| 4 | **RAG 工程完整度** | TARGET：可验证的完整 RAG 工程（自适应切分 + 混合检索 + 独立 Rerank + Parent Document + Recall@K/MRR/nDCG）。当前已实现多格式解析、多策略切分和 Dense+BM25+RRF；独立 Rerank 与版本化评估仍是 `Planned`，不得用本卖点宣称已有评测基线 |
...
├──────────────────────────────────────────────────────┤
│                    RAG Engine                         │
│  文档上传 → 多策略切分 → 清洗 → 向量化(Local 默认)    │
│  混合检索(BM25+向量+RRF) → 独立Rerank(Planned) → 引用 │
├──────────────────────────────────────────────────────┤
│              Workflow & Evolution                     │
...
|------|------------|------------|
| **Agent 对话** | ✅ 完整五阶段 | ✅ 同 OS |
| **RAG 知识库** | ✅ Local 默认 + 多格式解析（Milvus 可选 Partial，ADR-0002：正式 Local） | ✅ 目标：集群向量库与企业连接器（升格须另开 ADR） |
| **工作流引擎** | ✅ | ✅ |
| **记忆与进化** | ✅ | ✅ |
| **多租户** | ✅ | ✅ |
| **安全治理（6 层）** | ✅ | ✅ + 增强合规 |
...
| P1-2 | **混合检索 + 重排** | 当前：BM25 + 向量 + RRF 已落地（Implemented）。独立 Reranker 为 Planned，须在 RAG baseline 后单变量验证 |
